...
zss_editor.cleanupXXS = function(htmlcontent){
    var eventArray = [
                      'onerror',
                      'onload',
                      'onmove',
    ...
    var eventStr = eventArray.join('|');
    var regStr =  '(<[\\s]*?script[\\s]*?>[\\S\\s]*?<[\\s]*?/[\\s]*?script[\\s]*?>|<[\\s]*?iframe[^<>]*?[\\s\\S]*?>|<(\\s\\S)*?[^<>]*?(' + eventStr + ')[\\s\\S]*?>|<[\\s]*?style[\\s]*?>[\\S\\s]*?(javascript|expression)[\\S\\s]*?<[\\s]*?/[\\s]*?style[\\s]*?>)';
    var invalidRule = new RegExp(regStr, 'ig');
    var modifiedContent = htmlcontent;
    if (invalidRule.test(modifiedContent)) {
        modifiedContent = modifiedContent.replace(invalidRule, '');
    }
...
function mx_isHeader(t) {
    return (t == 'h1' || t == 'h2' || t == 'h3' || t == 'h4' || t == 'h5' || t == 'h6')
}

function mx_resetCursorPosition() {
    var range = document.createRange();
    var el = document.getSelection().focusNode;
...
zss_editor.setHeading = function(heading) {
    ...
    var is_heading = mx_isHeader(t);
    if (is_heading && heading == t) {
        var c = current_selection.html();
        current_selection.replaceWith(c);
        mx_resetCursorPosition();
    } else {
        if (t == 'li') {
            var element = current_selection.parent();
            var list, header;
            while (element) {
                var tag = element.prop('tagName').toLowerCase();
                if (tag == 'ol' || tag == 'ul') {
                    list = element;
                } else if (mx_isHeader(tag)) {
                    header = element;
                    break;
            ...
            var headingTag = heading.toLowerCase();
            if (header) {
                var content = header.html();
                if (header.prop('tagName').toLowerCase() != headingTag) {
                    content = `<${headingTag}>${content}</${headingTag}>`;
                }
                header.replaceWith(content);
            } else {
                list.wrap(`<${headingTag}></${headingTag}>`);
            }
            mx_resetCursorPosition();
        } else {
